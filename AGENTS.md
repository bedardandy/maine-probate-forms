...
Downloads Maine Probate Court PDF forms, detects fillable field regions using heuristics + VLM validation, and writes AcroForm widgets so the PDFs become truly fillable. Includes an MCP server for interactive field alignment via Codex.
...
> Two MCP servers ship here, don't confuse them: **`tools/agent_server.py`** is the forms-library server for agents (find_forms / get_form / fill_form) — use that to route and fill forms. **`mcp_server.py`** (below) is a *separate* tool for interactive PDF field-rect alignment.

A FastMCP server registered with Codex for interactive PDF field operations. Runs over stdio.
...
Verify with (`codex` or `claude`, depending on your client):
```bash
codex mcp list   # should show pdf-forms ✓ Connected
```

To register:
```bash
codex mcp add --transport stdio pdf-forms -- \
...
3. **Plan the fill:** build a canonical fact object `{matter, parties, party,
   ...
   `python3 tools/fill_plan.py --form <ID> --case case.json`. It resolves every
   field's `fill_strategy.source` into **resolved** `{field_id: value}` (from
   `case_dict.*` / `*_record.*`), a **narrative** worklist (the
   `llm_over_narrative` fields *you* compose), **recompute** (derived), **blank**
...
5. **Write the filled PDF:** fetch `source_url` (flat form), then
   ...
   rects`), so resolved text + checked options inject directly onto the flat
   source, with no pipeline at fill time (all 79 forms).
...
(`find_forms` / `get_form` / `fill_form`). Mirrors the companion
